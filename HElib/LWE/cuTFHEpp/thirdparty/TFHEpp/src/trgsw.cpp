...
#undef INST
    ...
    void trgswfftExternalProduct(TRLWE<P> &res, const TRLWE<P> &trlwe,
        ...
        for (int i = 1; i < P::l; i++) {
            ...
            for (int m = 0; m < P::k + 1; m++)
                FMAInFD<P::n>(restrlwefft[m], decpolyfft, trgswfft[i][m]); // restrlwefft += decpolyfft · trgswfft
        }
...
#undef INST
    ...
    TRGSW<P> trgswSymEncrypt(const Polynomial<P> &p, const double α,
        ...
        TRGSW<P> trgsw; // (k+1)*l trlwes, trlwe: k+1 polys
        for (TRLWE<P> &trlwe : trgsw)
            trlwe = trlweSymEncryptZero<P>(α, key);
        for (int i = 0; i < P::l; i++) {         // l = 3 or 4
            for (int k = 0; k < P::k + 1; k++) { // k = 0, 1
                for (int j = 0; j < P::n; j++) { // j is coeff_id, k is poly_size, n = 1024
                    trgsw[i + k * P::l][k][j] +=
                        static_cast<typename P::T>(p[j]) * h[i]; // TGSW_s(m) = Z + m · G^T \in T_{N,q}[x]^{(k+1)l x (k+1)}
                }
            }
            // 只有最后一个是 TLWE(0) + m·(0, ..., 0, 1 / B ^ l), 前面的是对应位置上有元素，poly p 是相同的，用 p 直接乘以 G^T
        }
        return trgsw;
    }
#define INST(P)                                                  \
