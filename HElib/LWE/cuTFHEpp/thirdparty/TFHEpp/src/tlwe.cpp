...
namespace TFHEpp {
    ...
    TLWE<P> tlweSymEncrypt(const typename P::T p, const double α, const Key<P> &key) {
        ...
        TLWE<P> res = {};
        res[P::k * P::n] = ModularGaussian<P>(p, α); // \mu* = \mu + e
        for (int k = 0; k < P::k; k++)
            for (int i = 0; i < P::n; i++) {
                res[k * P::n + i] = Torusdist(generator);
                res[P::k * P::n] += res[k * P::n + i] * key[k * P::n + i]; // b = \mu* + \sum_{i=0}^{n-1} a_i * s_i
            }
        return res;
    }
#define INST(P)                                                               \
...
#undef INST
    ...
    bool tlweSymDecrypt(const TLWE<P> &c, const Key<P> &key) {
        ...
        bool res =
            static_cast<typename std::make_signed<typename P::T>::type>(phase) > 0; // unsigned to signed
        return res;
    }
#define INST(P) \
