...
#define CHECK_CUDA_LAST_ERROR() checkLast(__FILE__, __LINE__)
...
namespace phantom::util {
    ...
    public:
        ...
        cuda_auto_ptr(const cuda_auto_ptr &obj) {
            PHANTOM_CHECK_CUDA(cudaMallocAsync((void **)&this->ptr_, obj.n_ * sizeof(T), obj.cudaStream_));
    ...
    cuda_auto_ptr<T> make_cuda_auto_ptr(size_t n, const cudaStream_t &stream) {
        T *ptr;
        PHANTOM_CHECK_CUDA(cudaMallocAsync((void **)&ptr, n * sizeof(T), stream));
    ...
    public:
        ...
        explicit CUDATimer(std::string func_name, const cudaStream_t &s) : func_name_(std::move(func_name)) {
            stream_ = s;
            cudaEventCreate(&start_event_);
            cudaEventCreate(&stop_event_);
        }
