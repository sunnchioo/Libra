...
namespace rlwe {
    ...
    public:
        ...
        template <typename T, typename = std::enable_if_t<std::is_same<std::remove_cv_t<T>, double>::value || std::is_same<std::remove_cv_t<T>, std::complex<double>>::value>>
        inline void decode_nonfft(PhantomPlaintext &plain, vector<T> &values) {
            encoder->decode_nonfft(*context, plain, values);
        }
    };
    ...
    public:
        ...
        Evaluator(PhantomContext *context) {
            this->context = context;
        ...
        inline void multiply_plain_inplace(PhantomCiphertext &ct, PhantomPlaintext &plain, const phantom::util::cuda_stream_wrapper &stream_wrapper) {
        ...
        inline void add_inplace(PhantomCiphertext &ct1, const PhantomCiphertext &ct2, const phantom::util::cuda_stream_wrapper &stream_wrapper) {
        ...
        inline void transform_from_ntt_inplace(uint64_t *data, size_t coeff_modulus_size, size_t start_modulus_idx, const cudaStream_t &stream) {
            nwt_2d_radix8_backward_inplace(data, context->gpu_rns_tables(), coeff_modulus_size, start_modulus_idx, stream);
        }

        inline void transform_from_1dntt_inplace(uint64_t *data, size_t coeff_modulus_size, size_t start_modulus_idx, const cudaStream_t &stream) {
            const auto &ntt_tables = context->gpu_rns_tables();
            inwt_1d_opt(data, ntt_tables.itwiddle(), ntt_tables.itwiddle_shoup(), ntt_tables.modulus(),
                        ntt_tables.n_inv_mod_q(), ntt_tables.n_inv_mod_q_shoup(), ntt_tables.n(), coeff_modulus_size, start_modulus_idx, stream);
        }
        ...
        inline void transform_to_ntt_inplace(uint64_t *data, size_t coeff_modulus_size, size_t start_modulus_idx, const cudaStream_t &stream) {
            nwt_2d_radix8_forward_inplace(data, context->gpu_rns_tables(), coeff_modulus_size, start_modulus_idx, stream);
        }

        inline void transform_to_1dntt_inplace(uint64_t *data, size_t coeff_modulus_size, size_t start_modulus_idx, const cudaStream_t &stream) {
            const auto &ntt_tables = context->gpu_rns_tables();
            fnwt_1d_opt(data, ntt_tables.twiddle(), ntt_tables.twiddle_shoup(), ntt_tables.modulus(), ntt_tables.n(), coeff_modulus_size, start_modulus_idx, stream);
        }

        // Bootstrapping
        inline void
        multiply_const(const PhantomCiphertext &ct, double value, PhantomCiphertext &dest) {
            dest = ct;
        ...
        inline void multiply_const_inplace(PhantomCiphertext &ct, double value) {
            ::multiply_const_inplace(*context, ct, value);
        }
        ...
        inline void format_switch_modrasing_new(PhantomCiphertext &ct) {
            ::format_switch_modrasing_inplace_new(*context, ct);
        }
        ...
        inline void add_const_inplace(PhantomCiphertext &ct, double value) {
            ::add_const_inplace(*context, ct, value);
        }
        ...
        inline void multiply_vector_reduced_error(PhantomCiphertext &ct, std::vector<T> &values, PhantomCiphertext &dest) {
            ...
            dest = ct;
            // std::cout << " before multiply_vector_inplace_reduced_error " << std::endl;
            multiply_vector_inplace_reduced_error(dest, values);
            // std::cout << " before multiply_vector_inplace_reduced_error done." << std::endl;
        }
        ...
        inline void multiply_vector_inplace_reduced_error(PhantomCiphertext &ct, vector<double> &values) {
            ...
            values.resize(encoder->slot_count(), 0.0);
            encoder->encode(*context, values, ct.scale(), plain, ct.chain_index());
            mod_switch_to_inplace(plain, ct.params_id());
            // std::cout << "chain index: " << ct.chain_index() << " " << plain.chain_index() << std::endl;
        ...
        inline void multiply_vector_inplace_reduced_error(PhantomCiphertext &ct, vector<complex<double>> &values) {
            ...
            // std::cout << "values: " << encoder->slot_count() << std::endl;
            encoder->encode(*context, values, ct.scale(), plain);
            // std::cout << " after encode " << std::endl;
            mod_switch_to_inplace(plain, ct.params_id());
            // std::cout << " after mod_switch_to_inplace " << std::endl;
            multiply_plain_inplace(ct, plain);
        }

        /************* new add ******************/
        inline void mult_image_unit(PhantomCiphertext &ct) {
            ::mult_image_unit_inplace(*context, ct);
        }

        inline void right_shift_inplace(PhantomCiphertext &ct, int shift) {
            ::right_shift_inplace(*context, ct, shift);
        }

        inline void many_rotate(PhantomCiphertext &ct, const PhantomGaloisKey &galois_keys, const std::vector<int> &steps) {
    ...
    public:
        ...
        const PhantomSecretKey *get_secretKey() const {
            return decryptor;
        }
        ...
        inline void create_galois_keys_from_steps(vector<int> &steps, PhantomGaloisKey &galois_keys) {
            galois_keys = decryptor->create_galois_keys_from_steps(*context, steps);  // vector of relin_keys_
        }
    ...
    public:
        ...
        CKKSEvaluator(PhantomContext *context, PhantomPublicKey *encryptor, PhantomSecretKey *decryptor,
            ...
            this->sparse_slots = encoder->sparse_slots();
        ...
        void print_decrypted_ct(PhantomCiphertext &ct, int num0, int num1, std::string str);
        void print_decrypted_ct_matirx(PhantomCiphertext &ct, int rows, int cols, std::string str);
        void print_decrypted_ct(PhantomCiphertext &ct, bool iscomplex, int num, std::string str);
        void print_decrypted_ct(PhantomCiphertext &ct, bool iscomplex, int num0, int num1, std::string str);
        double print_decrypted_ct_min(PhantomCiphertext &ct, int num, std::string str);

        void print_decoded_pt(PhantomPlaintext &pt, int num);
        void print_decoded_pt(PhantomPlaintext &pt, int num, std::string str);

        void print_decrypted_nodecode_ct(PhantomCiphertext &ct, int num, std::string str);
        void print_decrypted_nodecode_ct(PhantomCiphertext &ct, int num0, int num1, std::string str);

        void print_vector(vector<double> &in, int num, std::string str);
