...
public:
    ...
    DModulus(const uint64_t value, const uint64_t ratio0, const uint64_t ratio1) : value_(value), const_ratio_{ratio0, ratio1} {}
...
public:
    ...
    void set(const DModulus *modulus, const uint64_t *twiddle, const uint64_t *twiddle_shoup, const uint64_t *itwiddle,
        ...
        CHECK_CUDA_ERROR(cudaMemcpyAsync(modulus_.get() + index, modulus, sizeof(DModulus), cudaMemcpyHostToDevice, stream));
        cudaDeviceSynchronize();
        CHECK_CUDA_LAST_ERROR();
        CHECK_CUDA_ERROR(cudaMemcpyAsync(twiddle_.get() + index * n_, twiddle, n_ * sizeof(uint64_t), cudaMemcpyHostToDevice, stream));
        cudaDeviceSynchronize();
        CHECK_CUDA_LAST_ERROR();
        CHECK_CUDA_ERROR(cudaMemcpyAsync(twiddle_shoup_.get() + index * n_, twiddle_shoup, n_ * sizeof(uint64_t),
                                         cudaMemcpyHostToDevice, stream));
        cudaDeviceSynchronize();
        CHECK_CUDA_LAST_ERROR();
        CHECK_CUDA_ERROR(cudaMemcpyAsync(itwiddle_.get() + index * n_, itwiddle, n_ * sizeof(uint64_t), cudaMemcpyHostToDevice, stream));
        cudaDeviceSynchronize();
        CHECK_CUDA_LAST_ERROR();
        CHECK_CUDA_ERROR(cudaMemcpyAsync(itwiddle_shoup_.get() + index * n_, itwiddle_shoup, n_ * sizeof(uint64_t),
                                         cudaMemcpyHostToDevice, stream));
        cudaDeviceSynchronize();
        CHECK_CUDA_LAST_ERROR();
        CHECK_CUDA_ERROR(cudaMemcpyAsync(n_inv_mod_q_.get() + index, &n_inv_mod_q, sizeof(uint64_t), cudaMemcpyHostToDevice, stream));
        cudaDeviceSynchronize();
        CHECK_CUDA_LAST_ERROR();
        CHECK_CUDA_ERROR(cudaMemcpyAsync(n_inv_mod_q_shoup_.get() + index, &n_inv_mod_q_shoup, sizeof(uint64_t), cudaMemcpyHostToDevice,
                                         stream));
        cudaDeviceSynchronize();
        CHECK_CUDA_LAST_ERROR();
    }
