...
private:
    ...
    void decode_nonfft_internal(const PhantomContext &context, const PhantomPlaintext &plain,
...
public:
    ...
    inline void encode(const PhantomContext &context,
        ...
        destination.chain_index_ = 0;
        destination.resize(context.coeff_mod_size_, context.poly_degree_, s);
        encode_internal(context, value, chain_index, scale, destination, s);
    }
    ...
    template <class T>
    [[nodiscard]] inline auto encode(const PhantomContext &context, const T value,
                                     double scale,
                                     size_t chain_index = 1,  // first chain index
                                     const phantom::util::cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream) {
        PhantomPlaintext destination;
        encode(context, value, scale, destination, chain_index, stream_wrapper);
        return destination;
    }
    ...
    // template <class T>
    // [[nodiscard]] inline auto decode_nonfft(const PhantomContext &context, const PhantomPlaintext &plain,
    //                                         const phantom::util::cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream) {
    //     std::vector<T> destination;
    //     decode_nonfft_internal(context, plain, destination, stream_wrapper);
    //     return destination;
    // }

    template <class T>
    inline void decode_nonfft(const PhantomContext &context,
                              const PhantomPlaintext &plain,
                              std::vector<T> &destination,
                              const phantom::util::cuda_stream_wrapper &stream_wrapper = *phantom::util::global_variables::default_stream) {
        const auto &s = stream_wrapper.get_stream();
        auto decoding_sparse_slots = decoding_sparse_slots_ == 0 ? sparse_slots_ : decoding_sparse_slots_;
        destination.resize(decoding_sparse_slots);
        decode_nonfft_internal(context, plain, destination.data(), s);
    }
    ...
    inline void decode_nonfft(const PhantomContext &context,
        ...
        std::vector<cuDoubleComplex> output(decoding_sparse_slots);
        decode_nonfft_internal(context, plain, output.data(), s);
