...
public:
    ...
    void resize(const PhantomContext &context, size_t chain_index, size_t size, bool fill_zero, const cudaStream_t &stream) {
        auto &context_data = context.get_context_data(chain_index);
        auto &parms = context_data.parms();
        auto &coeff_modulus = parms.coeff_modulus();
        auto coeff_modulus_size = coeff_modulus.size();
        ...
        if (new_size != old_size) {
            ...
            data_ = phantom::util::make_cuda_auto_ptr<uint64_t>(size * coeff_modulus_size * poly_modulus_degree, stream);
            size_t copy_size = std::min(old_size, new_size);
            cudaMemcpyAsync(data_.get(), prev_data.get(), copy_size * sizeof(uint64_t), cudaMemcpyDeviceToDevice, stream);

            if (new_size > old_size) {
                // Initialize the new data to 0
                cudaMemsetAsync(data_.get() + copy_size, 0, (new_size - copy_size) * sizeof(uint64_t), stream);
            }
        }
    ...
    void resize(const PhantomContext &context, size_t chain_index, const cudaStream_t &stream) {
        ...
        // std::cout << "ct size: " << old_size << " " << new_size << std::endl;
    ...
    // void MallocData(size_t size, size_t coeff_modulus_size, size_t poly_modulus_degree, const cudaStream_t &stream) {
    //     size_t old_size = size_ * coeff_modulus_size_ * poly_modulus_degree_;
    //     size_t new_size = size * coeff_modulus_size * poly_modulus_degree;
    ...
    [[nodiscard]] auto &is_ntt_form() noexcept {
        return is_ntt_form_;
    }
    ...
    [[nodiscard]] auto &correction_factor() noexcept {
        return correction_factor_;
    }
