...
// rigth shift
__global__ void right_shift_rns_poly(const uint64_t *operand,
                                     const DModulus *modulus,
                                     uint64_t *result,
                                     const uint64_t poly_degree,
                                     const uint64_t coeff_mod_size, int shift_bits) {
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < poly_degree * coeff_mod_size; tid += blockDim.x * gridDim.x) {
        size_t twr = tid / poly_degree;
        DModulus mod_value = modulus[twr];
        uint64_t mod = mod_value.value();

        // uint64_t tmp = (operand[tid] >> shift_bits) & 0xe000000000000000;
        // result[tid] = (tmp + (operand[tid] & 0x1fffffffffffffff)) % mod;

        result[tid] = (operand[tid] / (1 << shift_bits)) % mod;
    }
}

// lwe decrypt
__global__ void
dot_product_mod_and_add(const uint64_t *operand0, const uint64_t *operand1, const DModulus *modulus,
                        uint64_t half_modulus, double scale, double *result) {
    const int tid = threadIdx.x;
    __shared__ uint64_t shared_sum[1024];  // lwe

    auto module = modulus[0];

    shared_sum[tid] = multiply_and_barrett_reduce_uint64(operand0[tid], operand1[tid], module.value(), module.const_ratio());

    __syncthreads();

    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) {
        if (tid < stride) {
            shared_sum[tid] = add_uint64_uint64_mod(shared_sum[tid], shared_sum[tid + stride], module.value());
        }
        __syncthreads();
    }

    if (tid == 0) {
        uint64_t acc = add_uint64_uint64_mod(shared_sum[0], operand1[1024], module.value());
        int64_t v = static_cast<int64_t>(acc);
        if (acc >= half_modulus) {
            v -= module.value();
        }
        *result = static_cast<double>(v) / scale;
    }
}

__global__ void kernel_modraise_inplace_(uint64_t *poly_dest, const uint64_t *poly_src_zero, const uint64_t modulus,
...
__global__ void modrasing_rns_poly(const uint64_t *operand,
                                   const DModulus *modulus,
                                   uint64_t *result,
                                   const uint64_t poly_degree,
                                   const uint64_t coeff_mod_size) {
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
         tid < poly_degree * coeff_mod_size;
         tid += blockDim.x * gridDim.x) {
        size_t poly_id = tid / poly_degree;
        size_t coeff_id = tid % poly_degree;

        uint64_t operand_tmp = operand[coeff_id];
        uint64_t q0 = modulus[0].value();
        DModulus mod_value = modulus[poly_id];
        uint64_t minus_q0 = 0;

        if (poly_id > 0) {
            minus_q0 = mod_value.value() - barrett_reduce_uint64_uint64(q0, mod_value.value(), mod_value.const_ratio()[1]);
            // minus_q0 = mod_value.value() - q0 % mod_value.value();
        }

        uint64_t tmp = barrett_reduce_uint64_uint64(operand_tmp, mod_value.value(), mod_value.const_ratio()[1]);
        // uint64_t tmp = operand_tmp % mod_value.value();
        if (operand_tmp > (q0 >> 1)) {
            tmp += minus_q0;
            tmp -= (tmp >= mod_value.value()) ? mod_value.value() : 0;
        }
...
__global__ void mult_image_rns_poly(const uint64_t *operand,
                                    uint64_t *result,
                                    const DModulus *modulus,
                                    const uint64_t poly_degree,
                                    const uint64_t coeff_mod_size) {
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
         tid < poly_degree * coeff_mod_size / 2;
         tid += blockDim.x * gridDim.x) {
        size_t poly_id = (2 * tid) / poly_degree;
        size_t coeff_id = poly_id * poly_degree + tid % (poly_degree / 2);

        uint64_t tmp0 = operand[coeff_id];
        uint64_t tmp1 = operand[coeff_id + poly_degree / 2];

        tmp1 = modulus[poly_id].value() - tmp1;

        result[coeff_id] = tmp1;
        result[coeff_id + poly_degree / 2] = tmp0;
    }
}

/**  res = (operand - scalar) % coeff_mod_list
 * @param[in] operand uint64_t
 * @param[in] DModulus Coeff modulus list
 * @param[out] result The buff to hold the result
 */
__global__ void sub_scalar_rns_poly(const uint64_t *operand,
                                    const uint64_t *scalar,
                                    const DModulus *modulus,
                                    uint64_t *result,
                                    const uint64_t poly_degree,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        result[tid] = sub_uint64_uint64_mod(operand[tid], scalar_value, mod_value.value());
    }
}
 ...
 * @param[out] result The buff to hold the result
 */
__global__ void add_scalar_rns_poly(const uint64_t *operand,
                                    const uint64_t *scalar,
                                    const DModulus *modulus,
                                    uint64_t *result,
                                    const uint64_t poly_degree,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        result[tid] = add_uint64_uint64_mod(operand[tid], scalar_value, mod_value.value());
    }
}

/**  res = operand % coeff_mod_list
 * @param[in] operand uint64_t
 * @param[in] DModulus Coeff modulus list
 * @param[out] result The buff to hold the result
 */
__global__ void const_rns_poly(const uint64_t operand,
                               const DModulus *modulus,
                               uint64_t *result,
                               const uint64_t coeff_mod_size) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < coeff_mod_size) {
        size_t twr = tid;
        DModulus mod = modulus[twr];

        result[tid] = barrett_reduce_uint64_uint64(operand, mod.value(), mod.const_ratio()[1]);
    }
}
...
__global__ void multiply_scalar_rns_poly(const uint64_t *operand,
    ...
    for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        uint64_t scalar_value = scalar[twr];
        if (flag) {
            result[tid] = multiply_and_barrett_reduce_uint64(operand[tid], scalar_value, mod_value.value(), mod_value.const_ratio());
        } else {
            result[tid] = multiply_and_barrett_reduce_uint64(operand[tid], mod_value.value() - scalar_value, mod_value.value(), mod_value.const_ratio());
        }
    }
}
// __global__ void multiply_scalar_rns_poly(const uint64_t *operand,
