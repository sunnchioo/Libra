...
void PhantomPublicKey::encrypt_zero_asymmetric_internal(const PhantomContext &context, PhantomCiphertext &cipher,
    ...
    if (prev_index == chain_index || chain_index != 1) {
...
void PhantomPublicKey::setpublickey(const uint64_t *key, const phantom::util::cuda_stream_wrapper &stream_wrapper) {
    ...
    auto poly_degree = pk_.poly_modulus_degree();
    auto modulus_size = pk_.coeff_modulus_size();
    auto size = pk_.size();
    size_t count = size * poly_degree * modulus_size;
    std::cout << "setpublickey: " << size << " " << poly_degree << " " << modulus_size << std::endl;
    const auto &s = stream_wrapper.get_stream();

    cudaMemsetAsync(pk_.data(), 0, count, s);
    cudaMemcpyAsync(pk_.data(), key, count, cudaMemcpyHostToDevice, s);
...
void PhantomSecretKey::encrypt_zero_symmetric(const PhantomContext &context, PhantomCiphertext &cipher,
    ...
    auto coeff_mod_size = coeff_modulus.size();
    auto base_rns = context.gpu_rns_tables().modulus();
    // std::cout << "resize in encrypt_zero_symmetric" << std::endl; out of memory error
    // std::cout << "poly_degree: " << poly_degree << " coeff_mod_size: " << coeff_mod_size << std::endl;

    // cudaDeviceSynchronize();
    // CHECK_CUDA_LAST_ERROR();
    cipher.resize(context, chain_index, 2, stream);  // size = 2
    // std::cout << "resize 1 in encrypt_zero_symmetric" << std::endl;
    ...
    // std::cout << "prng_seed_error in encrypt_zero_symmetric" << std::endl;
    auto prng_seed_error = make_cuda_auto_ptr<uint8_t>(phantom::util::global_variables::prng_seed_byte_count, stream);
    random_bytes(prng_seed_error.get(), phantom::util::global_variables::prng_seed_byte_count, stream);

    auto u = make_cuda_auto_ptr<uint64_t>(coeff_mod_size * poly_degree, stream);
    // std::cout << "u in encrypt_zero_symmetric" << std::endl;
    ...
    if (is_ntt_form) {
        ...
        if (poly_degree < (1 << 12)) {
            fnwt_1d_opt(u.get(), context.gpu_rns_tables().twiddle(), context.gpu_rns_tables().twiddle_shoup(), context.gpu_rns_tables().modulus(), poly_degree, coeff_mod_size, 0, stream);
        } else {
            nwt_2d_radix8_forward_inplace(u.get(), context.gpu_rns_tables(), coeff_mod_size, 0, stream);
        }
        ...
        if (poly_degree < (1 << 12)) {
            inwt_1d_opt(c0, context.gpu_rns_tables().itwiddle(), context.gpu_rns_tables().itwiddle_shoup(), context.gpu_rns_tables().modulus(),
                        context.gpu_rns_tables().n_inv_mod_q(), context.gpu_rns_tables().n_inv_mod_q_shoup(), poly_degree, coeff_mod_size, 0, stream);
        } else {
        ...
        // c1 backward
        if (poly_degree < (1 << 12)) {
            inwt_1d_opt(c1, context.gpu_rns_tables().itwiddle(), context.gpu_rns_tables().itwiddle_shoup(), context.gpu_rns_tables().modulus(),
                        context.gpu_rns_tables().n_inv_mod_q(), context.gpu_rns_tables().n_inv_mod_q_shoup(), poly_degree, coeff_mod_size, 0, stream);
        } else {
...
void PhantomSecretKey::generate_one_kswitch_key(const PhantomContext &context, uint64_t *new_key,
    ...
    // cudaDeviceSynchronize();
    // CHECK_CUDA_LAST_ERROR();
    // std::cout << " generate_one_kswitch_key 0 " << std::endl;
    ...
    CHECK_CUDA_ERROR(cudaMemcpyAsync(relin_keys.public_keys_ptr_.get(), pk_ptr.data(), sizeof(uint64_t *) * dnum, cudaMemcpyHostToDevice,
                                     stream));
...
// set secretkey form cpu pg
void PhantomSecretKey::set_secretkey(const uint64_t *secretkey_host, const cudaStream_t &stream) {
    const auto &s = stream;
    size_t count = poly_modulus_degree_ * coeff_modulus_size_;

    cudaMemsetAsync(secret_key_array_.get(), 0, count, s);
    cudaMemcpyAsync(secret_key_array_.get(), secretkey_host, count, cudaMemcpyHostToDevice, s);
    cudaStreamSynchronize(s);
}
...
void PhantomSecretKey::gen_secretkey(const PhantomContext &context, const cudaStream_t &stream) {
    ...
    // std::cout << "PhantomSecretKey Modulus: " << coeff_mod_size << " poly_degree: " << poly_degree << " " << std::endl;
    ...
    secret_key_non_ntt_array_ = make_cuda_auto_ptr<uint64_t>(poly_degree * coeff_mod_size, s);
    ...
    if (auto sk_hamming_weight = context.key_context_data().parms().secret_key_hamming_weight()) {
        ...
        cudaMemcpyAsync(sk_arr_non_ntt, secret_key_array_.get(), poly_degree * coeff_mod_size * sizeof(uint64_t), cudaMemcpyDeviceToHost, s);
        cudaStreamSynchronize(s);
        ...
        cudaMemcpyAsync(secret_key_array_.get(), sk_arr_non_ntt, poly_degree * coeff_mod_size * sizeof(uint64_t), cudaMemcpyHostToDevice, s);
    }

    // Compute the NTT form of secret key and
    // save secret_key to the first coeff_mod_size * N elements of secret_key_array
    cudaMemcpyAsync(secret_key_non_ntt_array_.get(), secret_key_array_.get(), poly_degree * coeff_mod_size * sizeof(uint64_t), cudaMemcpyDeviceToDevice, s);
    // {
    //     std::cout << "secret_key_non_ntt_array_: " << std::endl;
    //     uint64_t *rlwe_sk_re = new uint64_t[1024];
    //     cudaMemcpyAsync(rlwe_sk_re, secret_key_array_.get(), 1024 * sizeof(uint64_t), cudaMemcpyDeviceToHost, s);

    //     for (int i = 0; i < 1024; i++) {
    //         std::cout << rlwe_sk_re[i] << " ";
    //     }
    //     std::cout << std::endl;
    // }
...
// PhantomLweKey PhantomSecretKey::gen_lwekey(const PhantomContext &context) const { // lwe context
//     PhantomLweKey lwe_key;

//     const auto &s = phantom::util::global_variables::default_stream->get_stream();

//     lwe_key.gen_lwe_secretkey(context, s);

//     lwe_key.gen_flag_ = true;
...
PhantomGaloisKey PhantomSecretKey::create_galois_keys(const PhantomContext &context) const {
    ...
    const auto &s = phantom::util::global_variables::default_stream->get_stream();
    std::cout << "create galois keys. " << std::endl;
    ...
    auto relin_key_num = galois_elts.size();  // 47
    galois_keys.relin_keys_.resize(relin_key_num);
    // std::cout << "relin_keys size: " << relin_key_num << std::endl;
...
void PhantomSecretKey::create_galois_keys(const PhantomContext &context, PhantomGaloisKey &galois_keys,
    ...
    galois_keys.relin_key_num() = galois_elts.size();
    galois_keys.relin_keys_.resize(galois_keys.relin_key_num());

    for (size_t galois_elt_idx{0}; galois_elt_idx < galois_keys.relin_key_num(); galois_elt_idx++) {
        auto galois_elt = galois_elts[galois_elt_idx];
...
PhantomGaloisKey PhantomSecretKey::create_galois_keys_from_steps(PhantomContext &context, const std::vector<int> &steps) const {
    ...
    auto elts = context.key_galois_tool_->get_elts_from_steps(steps);    // 一个 step 就会有一个对应的 galois elt
    int log_n = phantom::arith::get_power_of_two(context.poly_degree_);  // log(degree) = 16
    bool is_bfv = (context.first_context_data().parms().scheme() == phantom::scheme_type::bfv);

    // std::cout << "elts: " << elts.size() << " steps: " << steps.size() << std::endl; // 47
    // for (auto iter : elts) {
    //     std::cout << iter << " ";
    // }
    // std::cout << std::endl;
...
// void PhantomSecretKey::lwe_decrypt(const PhantomContext &context, const PhantomCiphertext &encrypted, PhantomPlaintext &destination,
//                                    const cudaStream_t &stream) {
//     if (encrypted.is_ntt_form()) {
//         throw invalid_argument("encrypted must not be in NTT form");
//     }

//     const auto modulus = context.last_context_data().parms().coeff_modulus().front();
//     uint64_t half = modulus.value() >> 1;

//     const uint64_t *sk_data = secret_key_array();
//     const uint64_t *ct_data = encrypted.data(); // 有 n+1 个元素

//     dot_product_mod_and_add(sk_data, ct_data, modulus,
//                             half, double scale, double *result);
// }
