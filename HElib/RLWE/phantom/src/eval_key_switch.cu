...
void key_switch_inner_prod(uint64_t *p_cx, const uint64_t *p_t_mod_up, const uint64_t *const *rlk, const DRNSTool &rns_tool,
                           const DModulus *modulus_QP, size_t reduction_threshold, const cudaStream_t &stream) {

    const size_t size_QP = rns_tool.size_QP();
    const size_t size_P = rns_tool.size_P(); // alpha
    const size_t size_Q = size_QP - size_P;  // level
    ...
    // std::cout << "thread config:  " << size_QlP_n << " " << blockDimGlb.x << " " << size_QlP_n / blockDimGlb.x << std::endl;
    // std::cout << "key switch config: " << beta << " " << reduction_threshold << " " << size_P << " " << size_Q << " " << n << std::endl;
    // std::cout << "key switch config 1: " << size_Ql << " " << size_QlP << " " << std::endl;
    // dmum beta
    key_switch_inner_prod_c2_and_evk<<<size_QlP_n / blockDimGlb.x, blockDimGlb, 0, stream>>>(
        p_cx, p_t_mod_up, rlk, modulus_QP, n, size_QP, size_QP_n, size_QlP, size_QlP_n, size_Q, size_Ql, beta, reduction_threshold);
    // cudaDeviceSynchronize();
    // CHECK_CUDA_LAST_ERROR();
}
