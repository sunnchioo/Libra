...
namespace phantom::arith {
    void DRNSBase::init(const RNSBase &cpu_rns_base, bool isLWE, const cudaStream_t &stream) {
    ...
    void DRNSBase::decompose_double(uint64_t *dst, const double value, const uint32_t poly_degree,
                                    const uint32_t coeff_bit_count, const cudaStream_t &stream) const {
        ...
        if (coeff_bit_count <= 64) {
            decompose_uint64<<<gridDimGlb, blockDimGlb, 0, stream>>>(dst, coeffd, is_negative, base(), poly_degree, size());
        } else if (coeff_bit_count <= 128) {
            decompose_uint128<<<gridDimGlb, blockDimGlb, 0, stream>>>(dst, coeffd, is_negative, base(), poly_degree, size());
        } else {

            auto coeffu = make_cuda_auto_ptr<uint64_t>(size(), stream);
            ...
            decompose_uint<<<gridDimGlb, blockDimGlb, 0, stream>>>(dst, coeffu.get(), is_negative, base(), poly_degree, size());
        }
    }
