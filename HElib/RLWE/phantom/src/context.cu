...
namespace phantom {
    ...
    ContextData::ContextData(const EncryptionParameters &params, const cudaStream_t &stream) {
        ...
        // std::cout << "coeff modulus length: " << coeff_modulus.size() << std::endl;
        ...
        // std::cout << "Compute the product coeff modulus size: " << coeff_modulus_size << std::endl;
        ...
        // std::cout << "coeff modulus values size: " << coeff_modulus_values.size() << std::endl;
        ...
        // std::cout << "coeff modulus size: " << coeff_modulus_size << " bit count: " << total_coeff_modulus_bit_count_ << std::endl;
        // size_t poly_modulus_degree = params.poly_modulus_degree();
...
PhantomContext::PhantomContext(const phantom::EncryptionParameters &params) {
    ...
    if (phantom::util::global_variables::default_stream == nullptr) {
        // GPU setup
        int device;
        cudaGetDevice(&device);
        cudaMemPool_t mempool;
        CHECK_CUDA_ERROR(cudaDeviceGetDefaultMemPool(&mempool, device));
        if (mempool == nullptr) {
            throw std::runtime_error("获取默认内存池失败");
        }

        uint64_t threshold = UINT64_MAX;
        CHECK_CUDA_ERROR(cudaMemPoolSetAttribute(mempool, cudaMemPoolAttrReleaseThreshold, &threshold));
        // std::cout << "内存池自动释放阈值设置成功: " << threshold << " 字节 (30GB)" << std::endl;

        uint64_t actual_threshold;
        CHECK_CUDA_ERROR(cudaMemPoolGetAttribute(mempool, cudaMemPoolAttrReleaseThreshold, &actual_threshold));
        if (actual_threshold != threshold) {
            throw std::runtime_error("阈值设置不生效，实际值与预期不符");
        }
...
void PhantomContext::set_galois_elts(const std::vector<uint32_t> &galois_elts) {
    ...
    key_galois_tool_ = std::make_shared<PhantomGaloisTool>(galois_elts, log_n, s, is_bfv);
}