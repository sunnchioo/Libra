...
size_t FindLevelsToDrop(const PhantomContext &context, size_t multiplicativeDepth, double dcrtBits, bool isKeySwitch,
    ...
    std::cout << "context.using_keyswitching() is " << context.using_keyswitching() << std::endl;
...
void apply_galois_inplace(const PhantomContext &context, PhantomCiphertext &encrypted, uint32_t galois_elt,
    ...
    const auto &s = stream_wrapper.get_stream();
    // std::cout << "rotate_internal 1-0-1" << std::endl;

    auto temp = make_cuda_auto_ptr<uint64_t>(coeff_modulus_size * N, s);
    // std::cout << "rotate_internal 1-0-2" << std::endl;
    ...
    if (parms.scheme() == scheme_type::bfv) {
        ...
        // std::cout << "rotate_internal 1-0-3" << std::endl;
        ...
        // std::cout << "rotate_internal 1-0-4" << std::endl;

    } else {
        throw logic_error("scheme not implemented");
    }
    // std::cout << "rotate_internal 1-0-5" << std::endl;

    // Wipe c1
    cudaMemsetAsync(c1, 0, coeff_modulus_size * N * sizeof(uint64_t), s);

    ////////////////
    // std::cout << "galois_elt_index: " << galois_elt_index << " " << galois_keys.get_relin_keys_size() << std::endl;
    // std::cout << "key size: " << galois_keys.get_relin_keys(galois_elt_index).public_keys_ptr_size() << std::endl;
    // galois_keys.get_relin_keys(galois_elt_index) 应该少了一个
...
static void rotate_internal(const PhantomContext &context, PhantomCiphertext &encrypted, int step,
    ...
    if (iter != galois_elts.end()) {
        ...
        // std::cout << "has key: " << step << std::endl;

    } else {
        // std::cout << "rotate_internal 1-1" << std::endl;

        // Convert the steps to NAF: guarantees using smallest HW
        vector<int> naf_step = naf(step);
        // std::cout << "steps: " << step << " naf_steps.size: " << naf_step.size() << std::endl;
        // for (size_t i = 0; i < naf_step.size(); i++) {
        //     std::cout << naf_step[i] << " ";
        // }
        // std::cout << std::endl;
...
/**************** new kernel ********************/
static void ckks_multiply_const(const PhantomContext &context, PhantomCiphertext &encrypted, const double constant, const cudaStream_t &stream) {
    // Extract encryption parameters.
    auto &context_data = context.get_context_data(encrypted.chain_index());
    auto &parms = context_data.parms();
    auto &coeff_modulus = parms.coeff_modulus();
    ...
    double p = coeff_modulus[coeff_mod_size - 1].value();
    // double scFactor = std::pow(2.0, std::log(p));
    double scFactor = static_cast<double>(parms.coeff_modulus().back().value());
    bool flag = constant >= 0 ? 1 : 0;
    uint64_t large = static_cast<uint64_t>(std::abs(constant) * scFactor + 0.5);

    auto factor = make_cuda_auto_ptr<uint64_t>(coeff_mod_size, stream);

    uint64_t gridDimGlb = (coeff_mod_size + blockDimGlb.x - 1) / blockDimGlb.x;
    const_rns_poly<<<gridDimGlb, blockDimGlb, 0, stream>>>(large, base_rns, factor.get(), coeff_mod_size);

    for (size_t i = 0; i < encrypted.size(); i++) {
        gridDimGlb = poly_degree * coeff_mod_size / blockDimGlb.x;
        multiply_scalar_rns_poly<<<gridDimGlb, blockDimGlb, 0, stream>>>(
            encrypted.data() + i * rns_coeff_count, factor.get(), flag, base_rns,
            encrypted.data() + i * rns_coeff_count, poly_degree, coeff_mod_size);
    }

    encrypted.set_scale(encrypted.scale() * scFactor);
}

void multiply_const_inplace(const PhantomContext &context, PhantomCiphertext &encrypted, const double constant,
                            const phantom::util::cuda_stream_wrapper &stream_wrapper) {
    ...
    switch (context_data.parms().scheme()) {
    case scheme_type::bfv:
        throw std::logic_error("multiply_const Only CKKS is supported!");
        break;
    case scheme_type::ckks:
        ckks_multiply_const(context, encrypted, constant, s);
        break;
    case scheme_type::bgv:
        throw std::logic_error("multiply_const Only CKKS is supported!");
        break;
    ...
    default:
        throw invalid_argument("multiply_const Only CKKS is supported!");
    }
}

static void ckks_add_const(const PhantomContext &context, PhantomCiphertext &encrypted, const double constant, const cudaStream_t &stream) {
    // Extract encryption parameters.
    auto &context_data = context.get_context_data(encrypted.chain_index());
    auto &parms = context_data.parms();
    auto &coeff_modulus = parms.coeff_modulus();
    ...
    size_t p = coeff_modulus[coeff_mod_size - 1].value();
    double scFactor = std::pow(2.0, std::log2(p));
    // std::cout<< "ckks_add_const scFactor: "<<scFactor<<std::endl;

    uint64_t large = static_cast<uint64_t>(std::abs(constant) * scFactor + 0.5);

    auto factor = make_cuda_auto_ptr<uint64_t>(coeff_mod_size, stream);  // only constant>0
    ...
    if (constant > 0) {
        gridDimGlb = poly_degree * coeff_mod_size / blockDimGlb.x;
        add_scalar_rns_poly<<<gridDimGlb, blockDimGlb, 0, stream>>>(
            encrypted.data(), factor.get(), base_rns,
            encrypted.data(), poly_degree, coeff_mod_size);
    } else if (constant < 0) {
        gridDimGlb = poly_degree * coeff_mod_size / blockDimGlb.x;
        sub_scalar_rns_poly<<<gridDimGlb, blockDimGlb, 0, stream>>>(
            encrypted.data(), factor.get(), base_rns,
            encrypted.data(), poly_degree, coeff_mod_size);
    } else {
        return;
...
void add_const_inplace(const PhantomContext &context, PhantomCiphertext &encrypted, const double constant,
                       const phantom::util::cuda_stream_wrapper &stream_wrapper) {
    // const auto &s = cudaStreamPerThread;
    const auto &s = stream_wrapper.get_stream();
    auto &context_data = context.get_context_data(encrypted.chain_index());
    ...
    case scheme_type::bfv:
        ...
        break;
    case scheme_type::ckks:
        ckks_add_const(context, encrypted, constant, s);
        break;
    case scheme_type::bgv:
        throw std::logic_error("multiply_const Only CKKS is supported!");
        break;
...
void transform_to_ntt_inplace(const PhantomContext &context, PhantomCiphertext &ct, const cudaStream_t &stream) {
...
void transform_from_ntt_inplace(const PhantomContext &context, PhantomCiphertext &ct, const cudaStream_t &stream) {
    auto rns_coeff_count = ct.poly_modulus_degree() * ct.coeff_modulus_size();
...
static void ckks_switch_modrasing(const PhantomContext &context, PhantomCiphertext &cipher, const cudaStream_t &stream) {
    if (cipher.size() != 2) {
    ...
    if (cipher.is_ntt_form()) {
        transform_from_ntt_inplace(context, cipher, stream);
    }
    ...
    cipher.resize(context, context_data.chain_index(), stream);
    ...
    for (size_t poly_idx = 0; poly_idx < ciphertext_size; poly_idx++) {
        ...
        for (size_t j = 0; j < mod_count; j++) {
            ...
            kernel_modraise_inplace_<<<gridDimGlb, blockDimGlb, 0, stream>>>(
                poly_dest, rns_poly_src, modulus[j].value(), minus_q0[j], N, q0);
        }
    }

    transform_to_ntt_inplace(context, cipher, stream);
}

static void ckks_switch_modrasing_new(const PhantomContext &context, PhantomCiphertext &encrypted, const cudaStream_t &stream) {
    if (encrypted.size() != 2) {
        throw invalid_argument("Ciphertexts of size 2 are supported only!");
    }

    if (encrypted.coeff_modulus_size() != 1) {
        throw invalid_argument("Ciphertexts in the lowest level are supported only!");
    }

    // if (encrypted.is_ntt_form()) {
    //     transform_from_ntt_inplace(context, encrypted, stream);
    // }
    ...
    if (encrypted.is_ntt_form()) {
        ...
        // size_t max_chain_index = coeff_mod_size - 1;
        // auto &context_data_new = context.get_context_data(encrypted.chain_index());
        // auto &parms_new = context_data_new.parms();
        // auto &coeff_modulus_new = parms_new.coeff_modulus();
        // auto coeff_mod_size_new = coeff_modulus_new.size();
        // std::cout << "coeff_mod_size_new: " << coeff_mod_size_new << std::endl;

        // auto poly_degree_new = parms_new.poly_modulus_degree();
        // auto rns_coeff_count_new = poly_degree_new * coeff_mod_size_new;
        // std::cout << "ckks_switch_modrasing: " << coeff_mod_size_new << std::endl;
        // std::cout << "encrypted.chain_index(): " << encrypted.chain_index() << std::endl;
        // to coeff
        // encrypted.resize(context, max_chain_index, encrypted.size(), stream);

        // encrypted.resize(context, context_data.chain_index(), encrypted.size(), stream);
        encrypted.resize(context, context_data.chain_index(), stream);

        // level 0: 中心取模
        uint64_t gridDimGlb = poly_degree * coeff_mod_size / blockDimGlb.x;
        for (size_t i = 0; i < encrypted.size(); i++) {
            modrasing_rns_poly<<<gridDimGlb, blockDimGlb, 0, stream>>>(encrypted.data() + i * rns_coeff_count, base_rns,
                                                                       encrypted.data() + i * rns_coeff_count, poly_degree, coeff_mod_size);
        }

        transform_to_ntt_inplace(context, encrypted, stream);
    } else if (!encrypted.is_ntt_form()) {
        encrypted.resize(context, context_data.chain_index(), stream);
        for (size_t i = 0; i < encrypted.size(); i++) {
            for (size_t j = 1; j < coeff_mod_size; j++) {
                cudaMemcpyAsync(encrypted.data() + i * rns_coeff_count + j * poly_degree, encrypted.data() + i * rns_coeff_count,
    ...
    // encrypted.set_chain_index(1);
}

void format_switch_modrasing_inplace(const PhantomContext &context, PhantomCiphertext &encrypted,
                                     const phantom::util::cuda_stream_wrapper &stream_wrapper) {
    // const auto &s = cudaStreamPerThread;
    const auto &s = stream_wrapper.get_stream();
    auto &context_data = context.get_context_data(encrypted.chain_index());
    ...
    case scheme_type::bfv:
        ...
        break;
    case scheme_type::ckks:
        ckks_switch_modrasing(context, encrypted, s);
        break;
    case scheme_type::bgv:
        throw std::logic_error("multiply_const Only CKKS is supported!");
        break;
...
void format_switch_modrasing_inplace_new(const PhantomContext &context, PhantomCiphertext &encrypted,
                                         const phantom::util::cuda_stream_wrapper &stream_wrapper) {
    // const auto &s = cudaStreamPerThread;
    const auto &s = stream_wrapper.get_stream();
    auto &context_data = context.get_context_data(encrypted.chain_index());
    ...
    case scheme_type::bfv:
        ...
        break;
    case scheme_type::ckks:
        ckks_switch_modrasing_new(context, encrypted, s);
        break;
    case scheme_type::bgv:
        throw std::logic_error("multiply_const Only CKKS is supported!");
        break;
...
static void ckks_mult_image_unit(const PhantomContext &context, PhantomCiphertext &encrypted, const cudaStream_t &stream) {
    auto &context_data = context.get_context_data(encrypted.chain_index());
    auto &parms = context_data.parms();
    auto &coeff_modulus = parms.coeff_modulus();
    auto coeff_mod_size = coeff_modulus.size();
    ...
    uint64_t gridDimGlb = poly_degree * coeff_mod_size / (blockDimGlb.x * 2);

    if (encrypted.is_ntt_form()) {
        for (size_t i = 0; i < encrypted.size(); i++) {
            nwt_2d_radix8_backward_inplace(encrypted.data() + i * rns_coeff_count, context.gpu_rns_tables(), coeff_mod_size, 0, stream);
        }
    }
    for (size_t i = 0; i < encrypted.size(); i++) {
        mult_image_rns_poly<<<gridDimGlb, blockDimGlb, 0, stream>>>(
            encrypted.data() + i * rns_coeff_count, encrypted.data() + i * rns_coeff_count, base_rns,
            poly_degree, coeff_mod_size);
    }
    if (encrypted.is_ntt_form()) {
        for (size_t i = 0; i < encrypted.size(); i++) {
            nwt_2d_radix8_forward_inplace(encrypted.data() + i * rns_coeff_count, context.gpu_rns_tables(), coeff_mod_size, 0, stream);
        }
    }
}

void mult_image_unit_inplace(const PhantomContext &context, PhantomCiphertext &encrypted,
                             const phantom::util::cuda_stream_wrapper &stream_wrapper) {
    // const auto &s = cudaStreamPerThread;
    const auto &s = stream_wrapper.get_stream();
    ...
    case scheme_type::bfv:
        ...
        break;
    case scheme_type::ckks:
        ckks_mult_image_unit(context, encrypted, s);
        break;
    case scheme_type::bgv:
        throw std::logic_error("multiply_const Only CKKS is supported!");
        break;
...
static void right_shift_inplace(const PhantomContext &context, PhantomCiphertext &encrypted, int shift, const cudaStream_t &stream) {
    auto &context_data = context.get_context_data(encrypted.chain_index());
    auto &parms = context_data.parms();
    auto &coeff_modulus = parms.coeff_modulus();
    auto coeff_mod_size = coeff_modulus.size();
    ...
    // }
    for (size_t i = 0; i < encrypted.size(); i++) {
        right_shift_rns_poly<<<gridDimGlb, blockDimGlb, 0, stream>>>(
            encrypted.data() + i * rns_coeff_count, base_rns,
            encrypted.data() + i * rns_coeff_count, poly_degree, coeff_mod_size, shift);
    }
    // if (encrypted.is_ntt_form()) {
    //     for (size_t i = 0; i < encrypted.size(); i++) {
    //         nwt_2d_radix8_forward_inplace(encrypted.data() + i * rns_coeff_count, context.gpu_rns_tables(), coeff_mod_size, 0, stream);
...
void right_shift_inplace(const PhantomContext &context, PhantomCiphertext &encrypted, int shift, const phantom::util::cuda_stream_wrapper &stream_wrapper) {
    // const auto &s = cudaStreamPerThread;
    const auto &s = stream_wrapper.get_stream();
    ...
    switch (context_data.parms().scheme()) {
    case scheme_type::bfv:
        throw std::logic_error("right_shift Only CKKS is supported!");
        break;
    case scheme_type::ckks:
        right_shift_inplace(context, encrypted, shift, s);
        break;
    case scheme_type::bgv:
        throw std::logic_error("right_shift Only CKKS is supported!");
        break;
    ...
    default:
        throw invalid_argument("right_shift Only CKKS is supported!");
    }
}
