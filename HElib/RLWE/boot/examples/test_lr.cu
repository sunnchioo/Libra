...
int main() {
    ...
    long logN = 14; // 16 -> 15  // full only even(14)
    long loge = 10;
    ...
    // ckks_evaluator.evaluator.sub(cipher, cipher, cipher1);

    std::cout << "Slottocoeff..." << endl;
    bootstrapper.sfl_full_3(rtn, cipher);
    ...
    for (long i = 0; i < sparse_slots; i++) {
        if (i < 10)
            std::cout << before[i] << " <----> " << print_plain[i] << endl;
        mean_err += abs(before[i] - print_plain[i]);
    }
