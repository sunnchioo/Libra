...
void GELUEvaluator::gelu(PhantomCiphertext &x, PhantomCiphertext &res) {
    // cout << "Moduli before GELU: " << x.coeff_modulus_size() << endl;
    ckks->print_decrypted_ct(x, 10);
    ...
    ckks->print_decrypted_ct(b0, 10);
    // cout << "Moduli before sgn_eval: " << b0.coeff_modulus_size() << endl;
    b0 = ckks->sgn_eval(b0, d_g, d_f);
    // cout << "Moduli left after sgn_eval: " << b0.coeff_modulus_size() << endl;

    ckks->print_decrypted_ct(b0, 10);

    // std::cout << " sgn 1------------------------------------------------------- " << std::endl;
    // ckks->print_decrypted_ct(b1, 10);
