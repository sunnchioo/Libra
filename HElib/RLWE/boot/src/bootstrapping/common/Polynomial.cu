...
namespace boot {
    ...
    void Polynomial::set_polynomial(long _deg, RR *_coeff, string tag) {
        ...
        if (tag == "power") {
            ...
            power_to_cheb(); // 逐次消元法，由power basis得到chebyshev basis
        }
    ...
    void Polynomial::homomorphic_poly_evaluation(CKKSEvaluator *ckks, PhantomCiphertext &rtn, PhantomCiphertext &cipher) {
        // std::cout << "chipher scale: " << cipher.scale() << std::endl;
        ...
        if (deg == 1) {
            ...
            vector<PhantomCiphertext> baby(heap_k);
            ...
            vector<PhantomCiphertext> giant(heap_m);
            ...
            //     cout << i << " " << giant[i].scale() << " ";
            // }
            // cout << endl;

            // vector<PhantomCiphertext> cipherheap((1 << (heap_m + 1)) - 1, PhantomCiphertext());
            vector<PhantomCiphertext> cipherheap((1 << (heap_m + 1)) - 1);
            ...
            // cout << "cipherheap: " << endl;
            // for (int i = 0; i < cipherheap.size(); i++) {
            //     cout << i << " " << cipherheap[i].scale() << " ";
            // }
            // cout << endl;
    ...
    void chebyshev(Polynomial &rtn, long deg) {
        if (deg == 0) {
            ...
            Polynomial iden2(1);
            iden2.coeff[1] = RR(2); // 2x

            Polynomial tmp1(0), tmp2(1), tmp3;

            tmp1.coeff[0] = RR(1); // T_0(x) = 1
            tmp2.coeff[1] = RR(1); // T_1(x) = x

            for (int i = 2; i <= deg; i++) {
                mul(tmp3, iden2, tmp2); // 2x * T_{i-1}(x)
                subt(rtn, tmp3, tmp1);  // T_{i}(x) = 2x * T_{i-1}(x) - T_{i-2}(x)
                tmp1.copy(tmp2);
                tmp2.copy(rtn);
            }
        }
