...
namespace boot {
    ...
    // Remez 算法构造带误差项的逼近系统，使用 Chebyshev 多项式作为基底，但目标是得到极小极大意义下最优多项式逼近，不是做正交展开。
    ...
    void Remez::generate_optimal_poly(Polynomial &poly) {
        ...
        while ((max_err - min_err) / min_err > approx) {
            getcoeffwitherr();  // 求解切比雪夫系数，近似目标函数
            getextreme();       // 寻找当前逼近多项式的误差函数极值点
            choosemaxs();       // 筛选出满足交替极值条件的点，作为下一次迭代的采样点
            it++;
            // for(long i = 0; i < deg + 2; i++) {
        ...
        // showcoeff();
        poly.set_polynomial(deg, coeff, "cheb");  // 系数和切比雪夫系数都保存

        // std::ofstream out;
        // out.open("/mnt/data2/home/syt/data/fhec/log/graph.log");
        // RR step(0.1);
        // showgraph(out, coeff, deg, boundary_K, step);
        // exit(0);
    }
