...
/**
 * compare distance
 * 当 centers == 2 时，得到比较 bool 密文简单
 * 当 centers == 3 时，计算 d01 = d0 - d1, d02 = d0 - d2; d10 = d1 - d0, d12 = d1 - d2; d20 = d2 - d0, d21 = d2 - d1;
 *                     bool0 = d01 + d02, 其中离得近的还是负的; bool1 = d10 + d12; bool2 = d20 + d21;
 * 当 centers == 4 时，计算情况有些复杂。centers 0，计算 d01 = d0 - d1, d02 = d0 - d2; d03 = d0 - d3。
 *                     sgn(d01), sgn(d02), sgn(d03) 之后，变为(0, 1)的形式，然后相加，相与的部分为0，然后减0.5，再sgn+0.5，可获得与centers 0近的点(为0的部分)。
 */
...
int main() {
    ...
    std::vector<PhantomCiphertext> center_new_cipher(centers);
    ...
    for (size_t iter = 0; iter < iter_count; iter++) {
        ...
        // update center
        {
            ...
            timer.start();
            UpdateCenters(ckks_evaluator, center_cipher, points_cipher, centers, points, dim, bool_cipher); // 3 level
            timer.stop();
        }
        for (size_t idim = 0; idim < dim; idim++) {
            ckks_evaluator.print_decrypted_ct(center_cipher[idim], 10, "center " + std::to_string(idim));
        }
        cout << "center_cipher level: " << center_cipher[0].coeff_modulus_size() << endl; // 17
        ...
        cout << "center_cipher level: " << center_cipher[0].coeff_modulus_size() << endl;
    }
}
